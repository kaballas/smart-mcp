...
export class SmartsheetSheetAPI {
  ...
  async copySheet(
    ...
    if (destinationFolderId) {
      ...
      logger.debug('smartsheet-sheet-api', `Copying sheet to folder: ${destinationFolderId}`);
    } else if (workspaceId) {
      data.destinationType = 'workspace';
      data.destinationId = workspaceId;
      logger.debug('smartsheet-sheet-api', `Copying sheet to workspace: ${workspaceId}`);
    } else {
      // Default to 'home' if no folder or workspace specified
      data.destinationType = 'home';
      logger.debug('smartsheet-sheet-api', "Copying sheet to home");
    }
    
    const result = await this.api.request('POST', `/sheets/${sheetId}/copy`, data);
    logger.info('smartsheet-sheet-api', `Copy sheet result: ${JSON.stringify((result as any).result?.id)}`);
    return result;
  }
