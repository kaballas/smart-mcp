...
export function getDiscussionTools(server: McpServer, api: SmartsheetAPI) {
    ...
    server.tool(
        ...
        async ({ sheetId, include, pageSize, page, includeAll }) => {
            try {
                logger.info('discussion-tools', `Getting discussions for sheet with ID: ${sheetId}`);
                ...
                logger.error('discussion-tools', `Failed to get discussions for sheet ID: ${sheetId}`, { error });
                return {
                    content: [
                        {
                            type: "text",
    ...
    server.tool(
        ...
        async ({ sheetId, rowId, include, pageSize, page, includeAll }) => {
            try {
                logger.info('discussion-tools', `Getting discussions for row with ID: ${rowId} in sheet with ID: ${sheetId}`);
                ...
                logger.error('discussion-tools', `Failed to get discussions for row ID: ${rowId} in sheet ID: ${sheetId}`, { error });
                return {
                    content: [
                        {
                            type: "text",
    ...
    server.tool(
        ...
        async ({ sheetId, commentText }) => {
            try {
                logger.info('discussion-tools', `Creating discussion on sheet with ID: ${sheetId}`);
                ...
                logger.error('discussion-tools', `Failed to create discussion on sheet ID: ${sheetId}`, { error });
                return {
                    content: [
                        {
                            type: "text",
    ...
    server.tool(
        ...
        async ({ sheetId, rowId, commentText }) => {
            try {
                logger.info('discussion-tools', `Creating discussion on row with ID: ${rowId} in sheet with ID: ${sheetId}`);
                ...
                logger.error('discussion-tools', `Failed to create discussion on row ID: ${rowId} in sheet ID: ${sheetId}`, { error });
                return {
                    content: [
                        {
                            type: "text",
