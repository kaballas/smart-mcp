...
export function getFolderTools(server: McpServer, api: SmartsheetAPI) {
    ...
    server.tool(
        ...
        async ({ folderId}) => {
        try {
            logger.info('folder-tools', `Getting folder with ID: ${folderId}`);
            ...
            logger.error('folder-tools', `Failed to get folder with ID: ${folderId}`, { error });
            return {
            content: [
                {
                type: "text",
    ...
    server.tool(
        ...
        try {
            logger.info('folder-tools', `Creating folder in workspace with ID: ${folderId}`);
            ...
            logger.error('folder-tools', `Failed to create folder in workspace with ID: ${folderId}`, { error });
            return {
            content: [
                {
                type: "text",
    ...
    server.tool(
        ...
        try {
            logger.info('folder-tools', `Creating folder in workspace with ID: ${workspaceId}`);
            ...
            logger.error('folder-tools', `Failed to create folder in workspace with ID: ${workspaceId}`, { error });
            return {
            content: [
                {
                type: "text",
